//! Unit Economics (UNT) document implementation
//!
//! Per-customer or per-unit financial metrics
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.173315
...
impl UntDocument {
    /// Create a new Unit Economics document
...
impl BSpecDocument for UntDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be UNT-something)
        if !self.metadata.id.starts_with("UNT-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'UNT-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for UntDocument {
    fn default() -> Self {
        Self::new(
            "UNT-default".to_string(),
            "New Unit Economics".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = UntDocument::new(
            "UNT-test-001".to_string(),
            "Test Unit Economics".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "UNT-test-001");
        assert_eq!(doc.metadata.title, "Test Unit Economics");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Unt);
    }

    #[test]
    fn test_validation() {
        let mut doc = UntDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "UNT-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("UNT-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("UNT-ser-001"));

        // Test deserialization
        let deserialized: UntDocument = serde_json::from_str(&json).unwrap();
