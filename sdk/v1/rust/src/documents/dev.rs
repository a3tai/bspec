//! Development (DEV) document implementation
//!
//! Software development lifecycle and practices
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.177110
...
impl DevDocument {
    /// Create a new Development document
...
impl BSpecDocument for DevDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be DEV-something)
        if !self.metadata.id.starts_with("DEV-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'DEV-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for DevDocument {
    fn default() -> Self {
        Self::new(
            "DEV-default".to_string(),
            "New Development".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = DevDocument::new(
            "DEV-test-001".to_string(),
            "Test Development".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "DEV-test-001");
        assert_eq!(doc.metadata.title, "Test Development");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Dev);
    }

    #[test]
    fn test_validation() {
        let mut doc = DevDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "DEV-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("DEV-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("DEV-ser-001"));

        // Test deserialization
        let deserialized: DevDocument = serde_json::from_str(&json).unwrap();
