//! Facilities (FAC) document implementation
//!
//! Physical spaces, equipment, and infrastructure
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.175627
...
impl FacDocument {
    /// Create a new Facilities document
...
impl BSpecDocument for FacDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be FAC-something)
        if !self.metadata.id.starts_with("FAC-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'FAC-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for FacDocument {
    fn default() -> Self {
        Self::new(
            "FAC-default".to_string(),
            "New Facilities".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = FacDocument::new(
            "FAC-test-001".to_string(),
            "Test Facilities".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "FAC-test-001");
        assert_eq!(doc.metadata.title, "Test Facilities");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Fac);
    }

    #[test]
    fn test_validation() {
        let mut doc = FacDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "FAC-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("FAC-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("FAC-ser-001"));

        // Test deserialization
        let deserialized: FacDocument = serde_json::from_str(&json).unwrap();
