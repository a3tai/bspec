//! Moats (MOT) document implementation
//!
//! Competitive advantages that protect market position
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.163659
...
impl BSpecDocument for MotDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be MOT-something)
        if !self.metadata.id.starts_with("MOT-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'MOT-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for MotDocument {
    fn default() -> Self {
        Self::new(
            "MOT-default".to_string(),
            "New Moats".to_string(),
            "Unknown".to_string(),
        )
    }
...
mod tests {
    ...
    fn test_new_document() {
        let doc = MotDocument::new(
            "MOT-test-001".to_string(),
            "Test Moats".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "MOT-test-001");
        assert_eq!(doc.metadata.title, "Test Moats");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Mot);
    }

    #[test]
    fn test_validation() {
        let mut doc = MotDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "MOT-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("MOT-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("MOT-ser-001"));

        // Test deserialization
        let deserialized: MotDocument = serde_json::from_str(&json).unwrap();
