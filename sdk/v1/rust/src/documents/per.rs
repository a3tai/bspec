//! Performance (PER) document implementation
//!
//! Speed, reliability, and performance characteristics
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.169955
...
impl BSpecDocument for PerDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be PER-something)
        if !self.metadata.id.starts_with("PER-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'PER-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for PerDocument {
    fn default() -> Self {
        Self::new(
            "PER-default".to_string(),
            "New Performance".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = PerDocument::new(
            "PER-test-001".to_string(),
            "Test Performance".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "PER-test-001");
        assert_eq!(doc.metadata.title, "Test Performance");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Per);
    }

    #[test]
    fn test_validation() {
        let mut doc = PerDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "PER-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("PER-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("PER-ser-001"));

        // Test deserialization
        let deserialized: PerDocument = serde_json::from_str(&json).unwrap();
