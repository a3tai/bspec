//! Requirements (REQ) document implementation
//!
//! Functional and non-functional specifications
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.169462
...
impl BSpecDocument for ReqDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be REQ-something)
        if !self.metadata.id.starts_with("REQ-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'REQ-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for ReqDocument {
    fn default() -> Self {
        Self::new(
            "REQ-default".to_string(),
            "New Requirements".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = ReqDocument::new(
            "REQ-test-001".to_string(),
            "Test Requirements".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "REQ-test-001");
        assert_eq!(doc.metadata.title, "Test Requirements");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Req);
    }

    #[test]
    fn test_validation() {
        let mut doc = ReqDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "REQ-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("REQ-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("REQ-ser-001"));

        // Test deserialization
        let deserialized: ReqDocument = serde_json::from_str(&json).unwrap();
