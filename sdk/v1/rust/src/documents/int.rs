//! Integrations (INT) document implementation
//!
//! Connections with other systems and platforms
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.170118
...
impl BSpecDocument for IntDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be INT-something)
        if !self.metadata.id.starts_with("INT-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'INT-', got '{}'", self.metadata.id)
            ));
        }
...
mod tests {
    ...
    fn test_new_document() {
        let doc = IntDocument::new(
            "INT-test-001".to_string(),
            "Test Integrations".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "INT-test-001");
        assert_eq!(doc.metadata.title, "Test Integrations");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Int);
    }

    #[test]
    fn test_validation() {
        let mut doc = IntDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "INT-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("INT-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("INT-ser-001"));

        // Test deserialization
        let deserialized: IntDocument = serde_json::from_str(&json).unwrap();
