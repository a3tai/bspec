//! Roadmap (ROD) document implementation
//!
//! Planned development and enhancement timeline
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.169291
...
impl BSpecDocument for RodDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be ROD-something)
        if !self.metadata.id.starts_with("ROD-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'ROD-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for RodDocument {
    fn default() -> Self {
        Self::new(
            "ROD-default".to_string(),
            "New Roadmap".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = RodDocument::new(
            "ROD-test-001".to_string(),
            "Test Roadmap".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "ROD-test-001");
        assert_eq!(doc.metadata.title, "Test Roadmap");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Rod);
    }

    #[test]
    fn test_validation() {
        let mut doc = RodDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "ROD-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("ROD-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("ROD-ser-001"));

        // Test deserialization
        let deserialized: RodDocument = serde_json::from_str(&json).unwrap();
