//! Support (SUP) document implementation
//!
//! Customer support and success processes
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.170270
...
impl BSpecDocument for SupDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be SUP-something)
        if !self.metadata.id.starts_with("SUP-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'SUP-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for SupDocument {
    fn default() -> Self {
        Self::new(
            "SUP-default".to_string(),
            "New Support".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = SupDocument::new(
            "SUP-test-001".to_string(),
            "Test Support".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "SUP-test-001");
        assert_eq!(doc.metadata.title, "Test Support");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Sup);
    }

    #[test]
    fn test_validation() {
        let mut doc = SupDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "SUP-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("SUP-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("SUP-ser-001"));

        // Test deserialization
        let deserialized: SupDocument = serde_json::from_str(&json).unwrap();
