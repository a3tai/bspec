//! Skills (SKI) document implementation
//!
//! Required capabilities and competency frameworks
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.174875
...
impl BSpecDocument for SkiDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be SKI-something)
        if !self.metadata.id.starts_with("SKI-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'SKI-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for SkiDocument {
    fn default() -> Self {
        Self::new(
            "SKI-default".to_string(),
            "New Skills".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = SkiDocument::new(
            "SKI-test-001".to_string(),
            "Test Skills".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "SKI-test-001");
        assert_eq!(doc.metadata.title, "Test Skills");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Ski);
    }

    #[test]
    fn test_validation() {
        let mut doc = SkiDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "SKI-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("SKI-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("SKI-ser-001"));

        // Test deserialization
        let deserialized: SkiDocument = serde_json::from_str(&json).unwrap();
