//! Growth Model (GRW) document implementation
//!
//! How the business scales customers and revenue
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.181052
...
impl GrwDocument {
    /// Create a new Growth Model document
...
impl BSpecDocument for GrwDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be GRW-something)
        if !self.metadata.id.starts_with("GRW-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'GRW-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for GrwDocument {
    fn default() -> Self {
        Self::new(
            "GRW-default".to_string(),
            "New Growth Model".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = GrwDocument::new(
            "GRW-test-001".to_string(),
            "Test Growth Model".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "GRW-test-001");
        assert_eq!(doc.metadata.title, "Test Growth Model");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Grw);
    }

    #[test]
    fn test_validation() {
        let mut doc = GrwDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "GRW-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("GRW-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("GRW-ser-001"));

        // Test deserialization
        let deserialized: GrwDocument = serde_json::from_str(&json).unwrap();
