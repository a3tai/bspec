//! Capabilities (CAP) document implementation
//!
//! Organizational capabilities and competencies
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.175948
...
impl CapDocument {
    /// Create a new Capabilities document
...
impl BSpecDocument for CapDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be CAP-something)
        if !self.metadata.id.starts_with("CAP-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'CAP-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for CapDocument {
    fn default() -> Self {
        Self::new(
            "CAP-default".to_string(),
...
mod tests {
    ...
    fn test_new_document() {
        let doc = CapDocument::new(
            "CAP-test-001".to_string(),
            "Test Capabilities".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "CAP-test-001");
        assert_eq!(doc.metadata.title, "Test Capabilities");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Cap);
    }
    ...
    fn test_validation() {
        ...
        // Set required fields
        doc.metadata.id = "CAP-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("CAP-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("CAP-ser-001"));

        // Test deserialization
        let deserialized: CapDocument = serde_json::from_str(&json).unwrap();
