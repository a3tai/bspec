//! Market Segments (SEG) document implementation
//!
//! Customer categories and targeting strategy
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.164399
...
impl BSpecDocument for SegDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be SEG-something)
        if !self.metadata.id.starts_with("SEG-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'SEG-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for SegDocument {
    fn default() -> Self {
        Self::new(
            "SEG-default".to_string(),
            "New Market Segments".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = SegDocument::new(
            "SEG-test-001".to_string(),
            "Test Market Segments".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "SEG-test-001");
        assert_eq!(doc.metadata.title, "Test Market Segments");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Seg);
    }

    #[test]
    fn test_validation() {
        let mut doc = SegDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "SEG-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("SEG-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("SEG-ser-001"));

        // Test deserialization
        let deserialized: SegDocument = serde_json::from_str(&json).unwrap();
