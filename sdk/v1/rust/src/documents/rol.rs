//! Roles (ROL) document implementation
//!
//! Individual position definitions and responsibilities
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.174394
...
impl BSpecDocument for RolDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be ROL-something)
        if !self.metadata.id.starts_with("ROL-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'ROL-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for RolDocument {
    fn default() -> Self {
        Self::new(
            "ROL-default".to_string(),
            "New Roles".to_string(),
            "Unknown".to_string(),
        )
    }
...
mod tests {
    ...
    fn test_new_document() {
        let doc = RolDocument::new(
            "ROL-test-001".to_string(),
            "Test Roles".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "ROL-test-001");
        assert_eq!(doc.metadata.title, "Test Roles");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Rol);
    }

    #[test]
    fn test_validation() {
        let mut doc = RolDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "ROL-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("ROL-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("ROL-ser-001"));

        // Test deserialization
        let deserialized: RolDocument = serde_json::from_str(&json).unwrap();
