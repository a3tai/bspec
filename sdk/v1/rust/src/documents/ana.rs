//! Analytics (ANA) document implementation
//!
//! Data analytics, business intelligence, and insights
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.177279
...
impl BSpecDocument for AnaDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be ANA-something)
        if !self.metadata.id.starts_with("ANA-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'ANA-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for AnaDocument {
    fn default() -> Self {
        Self::new(
            "ANA-default".to_string(),
            "New Analytics".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = AnaDocument::new(
            "ANA-test-001".to_string(),
            "Test Analytics".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "ANA-test-001");
        assert_eq!(doc.metadata.title, "Test Analytics");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Ana);
    }

    #[test]
    fn test_validation() {
        let mut doc = AnaDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "ANA-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("ANA-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("ANA-ser-001"));

        // Test deserialization
        let deserialized: AnaDocument = serde_json::from_str(&json).unwrap();
