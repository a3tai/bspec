//! Crisis Management (CRI) document implementation
//!
//! Crisis response and business continuity plans
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.180246
...
impl CriDocument {
    /// Create a new Crisis Management document
...
impl BSpecDocument for CriDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be CRI-something)
        if !self.metadata.id.starts_with("CRI-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'CRI-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for CriDocument {
    fn default() -> Self {
        Self::new(
            "CRI-default".to_string(),
            "New Crisis Management".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = CriDocument::new(
            "CRI-test-001".to_string(),
            "Test Crisis Management".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "CRI-test-001");
        assert_eq!(doc.metadata.title, "Test Crisis Management");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Cri);
    }

    #[test]
    fn test_validation() {
        let mut doc = CriDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "CRI-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("CRI-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("CRI-ser-001"));

        // Test deserialization
        let deserialized: CriDocument = serde_json::from_str(&json).unwrap();
