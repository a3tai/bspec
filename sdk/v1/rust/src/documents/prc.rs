//! Processes (PRC) document implementation
//!
//! Repeatable workflows that drive business outcomes
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.173818
...
impl PrcDocument {
    /// Create a new Processes document
...
impl BSpecDocument for PrcDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be PRC-something)
        if !self.metadata.id.starts_with("PRC-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'PRC-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for PrcDocument {
    fn default() -> Self {
        Self::new(
            "PRC-default".to_string(),
            "New Processes".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = PrcDocument::new(
            "PRC-test-001".to_string(),
            "Test Processes".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "PRC-test-001");
        assert_eq!(doc.metadata.title, "Test Processes");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Prc);
    }

    #[test]
    fn test_validation() {
        let mut doc = PrcDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "PRC-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("PRC-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("PRC-ser-001"));

        // Test deserialization
        let deserialized: PrcDocument = serde_json::from_str(&json).unwrap();
