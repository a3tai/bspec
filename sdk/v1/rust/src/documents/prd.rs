//! Products (PRD) document implementation
//!
//! Physical or digital products offered
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.168384
...
impl PrdDocument {
    /// Create a new Products document
...
impl BSpecDocument for PrdDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be PRD-something)
        if !self.metadata.id.starts_with("PRD-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'PRD-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for PrdDocument {
    fn default() -> Self {
        Self::new(
            "PRD-default".to_string(),
            "New Products".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = PrdDocument::new(
            "PRD-test-001".to_string(),
            "Test Products".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "PRD-test-001");
        assert_eq!(doc.metadata.title, "Test Products");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Prd);
    }

    #[test]
    fn test_validation() {
        let mut doc = PrdDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "PRD-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("PRD-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("PRD-ser-001"));

        // Test deserialization
        let deserialized: PrdDocument = serde_json::from_str(&json).unwrap();
