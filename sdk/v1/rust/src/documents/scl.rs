//! Scaling (SCL) document implementation
//!
//! Operational scaling approach and constraints
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.181287
...
impl BSpecDocument for SclDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be SCL-something)
        if !self.metadata.id.starts_with("SCL-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'SCL-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for SclDocument {
    fn default() -> Self {
        Self::new(
            "SCL-default".to_string(),
            "New Scaling".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = SclDocument::new(
            "SCL-test-001".to_string(),
            "Test Scaling".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "SCL-test-001");
        assert_eq!(doc.metadata.title, "Test Scaling");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Scl);
    }
    ...
    fn test_validation() {
        ...
        // Set required fields
        doc.metadata.id = "SCL-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("SCL-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("SCL-ser-001"));

        // Test deserialization
        let deserialized: SclDocument = serde_json::from_str(&json).unwrap();
