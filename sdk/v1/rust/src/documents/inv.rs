//! Investment (INV) document implementation
//!
//! Capital allocation and investment decisions
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.178174
...
impl BSpecDocument for InvDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be INV-something)
        if !self.metadata.id.starts_with("INV-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'INV-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for InvDocument {
    fn default() -> Self {
        Self::new(
            "INV-default".to_string(),
            "New Investment".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = InvDocument::new(
            "INV-test-001".to_string(),
            "Test Investment".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "INV-test-001");
        assert_eq!(doc.metadata.title, "Test Investment");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Inv);
    }

    #[test]
    fn test_validation() {
        let mut doc = InvDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "INV-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("INV-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("INV-ser-001"));

        // Test deserialization
        let deserialized: InvDocument = serde_json::from_str(&json).unwrap();
