//! Financial Model (FIN) document implementation
//!
//! Comprehensive P&L, balance sheet, cash flow projections
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.177443
...
impl FinDocument {
    /// Create a new Financial Model document
...
impl BSpecDocument for FinDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be FIN-something)
        if !self.metadata.id.starts_with("FIN-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'FIN-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for FinDocument {
    fn default() -> Self {
        Self::new(
            "FIN-default".to_string(),
            "New Financial Model".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = FinDocument::new(
            "FIN-test-001".to_string(),
            "Test Financial Model".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "FIN-test-001");
        assert_eq!(doc.metadata.title, "Test Financial Model");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Fin);
    }

    #[test]
    fn test_validation() {
        let mut doc = FinDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "FIN-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("FIN-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("FIN-ser-001"));

        // Test deserialization
        let deserialized: FinDocument = serde_json::from_str(&json).unwrap();
