//! Pain Points (PAI) document implementation
//!
//! Customer problems and frustrations being solved
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.167250
...
impl PaiDocument {
    /// Create a new Pain Points document
...
impl BSpecDocument for PaiDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be PAI-something)
        if !self.metadata.id.starts_with("PAI-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'PAI-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for PaiDocument {
    fn default() -> Self {
        Self::new(
            "PAI-default".to_string(),
            "New Pain Points".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = PaiDocument::new(
            "PAI-test-001".to_string(),
            "Test Pain Points".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "PAI-test-001");
        assert_eq!(doc.metadata.title, "Test Pain Points");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Pai);
    }

    #[test]
    fn test_validation() {
        let mut doc = PaiDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "PAI-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("PAI-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("PAI-ser-001"));

        // Test deserialization
        let deserialized: PaiDocument = serde_json::from_str(&json).unwrap();
