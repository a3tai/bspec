//! Compliance (CMP) document implementation
//!
//! Regulatory obligations and adherence requirements
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.179506
...
impl BSpecDocument for CmpDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be CMP-something)
        if !self.metadata.id.starts_with("CMP-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'CMP-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for CmpDocument {
    fn default() -> Self {
        Self::new(
            "CMP-default".to_string(),
            "New Compliance".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = CmpDocument::new(
            "CMP-test-001".to_string(),
            "Test Compliance".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "CMP-test-001");
        assert_eq!(doc.metadata.title, "Test Compliance");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Cmp);
    }

    #[test]
    fn test_validation() {
        let mut doc = CmpDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "CMP-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("CMP-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("CMP-ser-001"));

        // Test deserialization
        let deserialized: CmpDocument = serde_json::from_str(&json).unwrap();
