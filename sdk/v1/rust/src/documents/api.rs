//! APIs (API) document implementation
//!
//! Interface specifications, protocols, and integrations
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.176613
...
impl BSpecDocument for ApiDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be API-something)
        if !self.metadata.id.starts_with("API-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'API-', got '{}'", self.metadata.id)
            ));
        }
...
mod tests {
    ...
    fn test_new_document() {
        let doc = ApiDocument::new(
            "API-test-001".to_string(),
            "Test APIs".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "API-test-001");
        assert_eq!(doc.metadata.title, "Test APIs");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Api);
    }
    ...
    fn test_validation() {
        ...
        // Set required fields
        doc.metadata.id = "API-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("API-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("API-ser-001"));
