//! Budget (BUD) document implementation
//!
//! Resource allocation and spending plans
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.177607
...
impl BSpecDocument for BudDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be BUD-something)
        if !self.metadata.id.starts_with("BUD-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'BUD-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for BudDocument {
    fn default() -> Self {
        Self::new(
            "BUD-default".to_string(),
...
mod tests {
    ...
    fn test_new_document() {
        let doc = BudDocument::new(
            "BUD-test-001".to_string(),
            "Test Budget".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "BUD-test-001");
        assert_eq!(doc.metadata.title, "Test Budget");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Bud);
    }

    #[test]
    fn test_validation() {
        let mut doc = BudDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "BUD-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("BUD-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("BUD-ser-001"));

        // Test deserialization
        let deserialized: BudDocument = serde_json::from_str(&json).unwrap();
