//! Tax Strategy (TAX) document implementation
//!
//! Tax planning, structure, and compliance
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.179005
...
impl TaxDocument {
    /// Create a new Tax Strategy document
...
impl BSpecDocument for TaxDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be TAX-something)
        if !self.metadata.id.starts_with("TAX-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'TAX-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for TaxDocument {
    fn default() -> Self {
        Self::new(
            "TAX-default".to_string(),
            "New Tax Strategy".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = TaxDocument::new(
            "TAX-test-001".to_string(),
            "Test Tax Strategy".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "TAX-test-001");
        assert_eq!(doc.metadata.title, "Test Tax Strategy");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Tax);
    }

    #[test]
    fn test_validation() {
        let mut doc = TaxDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "TAX-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("TAX-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("TAX-ser-001"));

        // Test deserialization
        let deserialized: TaxDocument = serde_json::from_str(&json).unwrap();
