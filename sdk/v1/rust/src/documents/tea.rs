//! Teams (TEA) document implementation
//!
//! Team structures, compositions, and dynamics
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.174684
...
impl BSpecDocument for TeaDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be TEA-something)
        if !self.metadata.id.starts_with("TEA-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'TEA-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for TeaDocument {
    fn default() -> Self {
        Self::new(
            "TEA-default".to_string(),
            "New Teams".to_string(),
            "Unknown".to_string(),
        )
    }
...
mod tests {
    ...
    fn test_new_document() {
        let doc = TeaDocument::new(
            "TEA-test-001".to_string(),
            "Test Teams".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "TEA-test-001");
        assert_eq!(doc.metadata.title, "Test Teams");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Tea);
    }

    #[test]
    fn test_validation() {
        let mut doc = TeaDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "TEA-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("TEA-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("TEA-ser-001"));

        // Test deserialization
        let deserialized: TeaDocument = serde_json::from_str(&json).unwrap();
