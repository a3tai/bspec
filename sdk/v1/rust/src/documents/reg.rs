//! Regulatory Environment (REG) document implementation
//!
//! Laws, regulations, and compliance requirements
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.165818
...
/// Regulatory Environment document
///
/// Laws, regulations, and compliance requirements
///
/// **Domain:** Market
...
impl RegDocument {
    /// Create a new Regulatory Environment document
...
impl BSpecDocument for RegDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be REG-something)
        if !self.metadata.id.starts_with("REG-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'REG-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for RegDocument {
    fn default() -> Self {
        Self::new(
            "REG-default".to_string(),
            "New Regulatory Environment".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = RegDocument::new(
            "REG-test-001".to_string(),
            "Test Regulatory Environment".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "REG-test-001");
        assert_eq!(doc.metadata.title, "Test Regulatory Environment");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Reg);
    }

    #[test]
    fn test_validation() {
        let mut doc = RegDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "REG-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("REG-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("REG-ser-001"));

        // Test deserialization
        let deserialized: RegDocument = serde_json::from_str(&json).unwrap();
