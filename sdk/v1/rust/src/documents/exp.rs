//! Expansion (EXP) document implementation
//!
//! Geographic, market, or product expansion plans
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.182294
...
impl ExpDocument {
    /// Create a new Expansion document
...
impl BSpecDocument for ExpDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be EXP-something)
        if !self.metadata.id.starts_with("EXP-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'EXP-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for ExpDocument {
    fn default() -> Self {
        Self::new(
            "EXP-default".to_string(),
            "New Expansion".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = ExpDocument::new(
            "EXP-test-001".to_string(),
            "Test Expansion".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "EXP-test-001");
        assert_eq!(doc.metadata.title, "Test Expansion");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Exp);
    }

    #[test]
    fn test_validation() {
        let mut doc = ExpDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "EXP-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("EXP-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("EXP-ser-001"));

        // Test deserialization
        let deserialized: ExpDocument = serde_json::from_str(&json).unwrap();
