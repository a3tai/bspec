//! Key Activities (ACT) document implementation
//!
//! Essential processes for value creation
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.172943
...
impl ActDocument {
    /// Create a new Key Activities document
...
impl BSpecDocument for ActDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be ACT-something)
        if !self.metadata.id.starts_with("ACT-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'ACT-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for ActDocument {
    fn default() -> Self {
        Self::new(
            "ACT-default".to_string(),
            "New Key Activities".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = ActDocument::new(
            "ACT-test-001".to_string(),
            "Test Key Activities".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "ACT-test-001");
        assert_eq!(doc.metadata.title, "Test Key Activities");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Act);
    }

    #[test]
    fn test_validation() {
        let mut doc = ActDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "ACT-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("ACT-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("ACT-ser-001"));

        // Test deserialization
        let deserialized: ActDocument = serde_json::from_str(&json).unwrap();
