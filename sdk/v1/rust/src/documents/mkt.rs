//! Market Definition (MKT) document implementation
//!
//! TAM/SAM/SOM, market boundaries and sizing
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.164155
...
impl MktDocument {
    /// Create a new Market Definition document
...
impl BSpecDocument for MktDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be MKT-something)
        if !self.metadata.id.starts_with("MKT-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'MKT-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for MktDocument {
    fn default() -> Self {
        Self::new(
            "MKT-default".to_string(),
            "New Market Definition".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = MktDocument::new(
            "MKT-test-001".to_string(),
            "Test Market Definition".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "MKT-test-001");
        assert_eq!(doc.metadata.title, "Test Market Definition");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Mkt);
    }

    #[test]
    fn test_validation() {
        let mut doc = MktDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "MKT-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("MKT-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("MKT-ser-001"));

        // Test deserialization
        let deserialized: MktDocument = serde_json::from_str(&json).unwrap();
