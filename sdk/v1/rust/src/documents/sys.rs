//! Systems (SYS) document implementation
//!
//! Software systems, platforms, and applications
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.176285
...
impl BSpecDocument for SysDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be SYS-something)
        if !self.metadata.id.starts_with("SYS-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'SYS-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for SysDocument {
    fn default() -> Self {
        Self::new(
            "SYS-default".to_string(),
            "New Systems".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = SysDocument::new(
            "SYS-test-001".to_string(),
            "Test Systems".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "SYS-test-001");
        assert_eq!(doc.metadata.title, "Test Systems");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Sys);
    }

    #[test]
    fn test_validation() {
        let mut doc = SysDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "SYS-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("SYS-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("SYS-ser-001"));

        // Test deserialization
        let deserialized: SysDocument = serde_json::from_str(&json).unwrap();
