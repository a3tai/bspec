//! Infrastructure (INF) document implementation
//!
//! Deployment, hosting, and runtime environment
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.176784
...
impl InfDocument {
    /// Create a new Infrastructure document
...
impl BSpecDocument for InfDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be INF-something)
        if !self.metadata.id.starts_with("INF-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'INF-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for InfDocument {
    fn default() -> Self {
        Self::new(
            "INF-default".to_string(),
            "New Infrastructure".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = InfDocument::new(
            "INF-test-001".to_string(),
            "Test Infrastructure".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "INF-test-001");
        assert_eq!(doc.metadata.title, "Test Infrastructure");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Inf);
    }

    #[test]
    fn test_validation() {
        let mut doc = InfDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "INF-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("INF-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("INF-ser-001"));

        // Test deserialization
        let deserialized: InfDocument = serde_json::from_str(&json).unwrap();
