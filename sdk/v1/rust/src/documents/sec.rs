//! Security (SEC) document implementation
//!
//! Security architecture, controls, and compliance
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.176954
...
impl BSpecDocument for SecDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be SEC-something)
        if !self.metadata.id.starts_with("SEC-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'SEC-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for SecDocument {
    fn default() -> Self {
        Self::new(
            "SEC-default".to_string(),
            "New Security".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = SecDocument::new(
            "SEC-test-001".to_string(),
            "Test Security".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "SEC-test-001");
        assert_eq!(doc.metadata.title, "Test Security");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Sec);
    }

    #[test]
    fn test_validation() {
        let mut doc = SecDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "SEC-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("SEC-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("SEC-ser-001"));

        // Test deserialization
        let deserialized: SecDocument = serde_json::from_str(&json).unwrap();
