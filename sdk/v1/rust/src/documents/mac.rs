//! Macro Environment (MAC) document implementation
//!
//! Economic, political, social, technological factors
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.165976
...
impl MacDocument {
    /// Create a new Macro Environment document
...
impl BSpecDocument for MacDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be MAC-something)
        if !self.metadata.id.starts_with("MAC-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'MAC-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for MacDocument {
    fn default() -> Self {
        Self::new(
            "MAC-default".to_string(),
            "New Macro Environment".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = MacDocument::new(
            "MAC-test-001".to_string(),
            "Test Macro Environment".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "MAC-test-001");
        assert_eq!(doc.metadata.title, "Test Macro Environment");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Mac);
    }

    #[test]
    fn test_validation() {
        let mut doc = MacDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "MAC-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("MAC-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("MAC-ser-001"));

        // Test deserialization
        let deserialized: MacDocument = serde_json::from_str(&json).unwrap();
