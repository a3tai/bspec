//! Mission (MSN) document implementation
//!
//! Why the organization exists and its core purpose
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.162655
...
impl BSpecDocument for MsnDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be MSN-something)
        if !self.metadata.id.starts_with("MSN-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'MSN-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for MsnDocument {
    fn default() -> Self {
        Self::new(
            "MSN-default".to_string(),
...
mod tests {
    ...
    fn test_new_document() {
        let doc = MsnDocument::new(
            "MSN-test-001".to_string(),
            "Test Mission".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "MSN-test-001");
        assert_eq!(doc.metadata.title, "Test Mission");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Msn);
    }

    #[test]
    fn test_validation() {
        let mut doc = MsnDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "MSN-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("MSN-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("MSN-ser-001"));

        // Test deserialization
        let deserialized: MsnDocument = serde_json::from_str(&json).unwrap();
