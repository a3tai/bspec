//! Purpose (PUR) document implementation
//!
//! Social impact and stakeholder value beyond profit
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.163824
...
impl BSpecDocument for PurDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be PUR-something)
        if !self.metadata.id.starts_with("PUR-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'PUR-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for PurDocument {
    fn default() -> Self {
        Self::new(
            "PUR-default".to_string(),
            "New Purpose".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = PurDocument::new(
            "PUR-test-001".to_string(),
            "Test Purpose".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "PUR-test-001");
        assert_eq!(doc.metadata.title, "Test Purpose");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Pur);
    }

    #[test]
    fn test_validation() {
        let mut doc = PurDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "PUR-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("PUR-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("PUR-ser-001"));

        // Test deserialization
        let deserialized: PurDocument = serde_json::from_str(&json).unwrap();
