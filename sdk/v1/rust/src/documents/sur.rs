//! Surveys (SUR) document implementation
//!
//! Quantitative customer research and data
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.168057
...
impl BSpecDocument for SurDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be SUR-something)
        if !self.metadata.id.starts_with("SUR-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'SUR-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for SurDocument {
    fn default() -> Self {
        Self::new(
            "SUR-default".to_string(),
            "New Surveys".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = SurDocument::new(
            "SUR-test-001".to_string(),
            "Test Surveys".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "SUR-test-001");
        assert_eq!(doc.metadata.title, "Test Surveys");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Sur);
    }

    #[test]
    fn test_validation() {
        let mut doc = SurDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "SUR-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("SUR-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("SUR-ser-001"));

        // Test deserialization
        let deserialized: SurDocument = serde_json::from_str(&json).unwrap();
