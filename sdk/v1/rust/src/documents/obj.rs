//! Objectives (OBJ) document implementation
//!
//! Specific, measurable goals with timeframes (OKRs)
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.163501
...
impl BSpecDocument for ObjDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be OBJ-something)
        if !self.metadata.id.starts_with("OBJ-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'OBJ-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for ObjDocument {
    fn default() -> Self {
        Self::new(
            "OBJ-default".to_string(),
            "New Objectives".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = ObjDocument::new(
            "OBJ-test-001".to_string(),
            "Test Objectives".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "OBJ-test-001");
        assert_eq!(doc.metadata.title, "Test Objectives");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Obj);
    }
    ...
    fn test_validation() {
        ...
        // Set required fields
        doc.metadata.id = "OBJ-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("OBJ-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("OBJ-ser-001"));

        // Test deserialization
        let deserialized: ObjDocument = serde_json::from_str(&json).unwrap();
