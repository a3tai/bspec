//! Key Resources (RES) document implementation
//!
//! Critical assets required for the business model
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.172730
...
impl ResDocument {
    /// Create a new Key Resources document
...
impl BSpecDocument for ResDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be RES-something)
        if !self.metadata.id.starts_with("RES-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'RES-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for ResDocument {
    fn default() -> Self {
        Self::new(
            "RES-default".to_string(),
            "New Key Resources".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = ResDocument::new(
            "RES-test-001".to_string(),
            "Test Key Resources".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "RES-test-001");
        assert_eq!(doc.metadata.title, "Test Key Resources");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Res);
    }

    #[test]
    fn test_validation() {
        let mut doc = ResDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "RES-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("RES-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("RES-ser-001"));

        // Test deserialization
        let deserialized: ResDocument = serde_json::from_str(&json).unwrap();
