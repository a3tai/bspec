//! Business Model Canvas (BMC) document implementation
//!
//! Complete business model overview
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.170429
...
impl BmcDocument {
    /// Create a new Business Model Canvas document
...
impl BSpecDocument for BmcDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be BMC-something)
        if !self.metadata.id.starts_with("BMC-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'BMC-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for BmcDocument {
    fn default() -> Self {
        Self::new(
            "BMC-default".to_string(),
            "New Business Model Canvas".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = BmcDocument::new(
            "BMC-test-001".to_string(),
            "Test Business Model Canvas".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "BMC-test-001");
        assert_eq!(doc.metadata.title, "Test Business Model Canvas");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Bmc);
    }

    #[test]
    fn test_validation() {
        let mut doc = BmcDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "BMC-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("BMC-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("BMC-ser-001"));

        // Test deserialization
        let deserialized: BmcDocument = serde_json::from_str(&json).unwrap();
