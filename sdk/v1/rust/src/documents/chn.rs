//! Channels (CHN) document implementation
//!
//! Distribution and sales channel strategy
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.171956
...
impl ChnDocument {
    /// Create a new Channels document
...
impl BSpecDocument for ChnDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be CHN-something)
        if !self.metadata.id.starts_with("CHN-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'CHN-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for ChnDocument {
    fn default() -> Self {
        Self::new(
            "CHN-default".to_string(),
            "New Channels".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = ChnDocument::new(
            "CHN-test-001".to_string(),
            "Test Channels".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "CHN-test-001");
        assert_eq!(doc.metadata.title, "Test Channels");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Chn);
    }

    #[test]
    fn test_validation() {
        let mut doc = ChnDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "CHN-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("CHN-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("CHN-ser-001"));

        // Test deserialization
        let deserialized: ChnDocument = serde_json::from_str(&json).unwrap();
