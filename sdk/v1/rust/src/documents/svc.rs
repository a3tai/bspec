//! Services (SVC) document implementation
//!
//! Service offerings and support capabilities
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.168537
...
impl BSpecDocument for SvcDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be SVC-something)
        if !self.metadata.id.starts_with("SVC-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'SVC-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for SvcDocument {
    fn default() -> Self {
        Self::new(
            "SVC-default".to_string(),
            "New Services".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = SvcDocument::new(
            "SVC-test-001".to_string(),
            "Test Services".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "SVC-test-001");
        assert_eq!(doc.metadata.title, "Test Services");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Svc);
    }

    #[test]
    fn test_validation() {
        let mut doc = SvcDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "SVC-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("SVC-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("SVC-ser-001"));

        // Test deserialization
        let deserialized: SvcDocument = serde_json::from_str(&json).unwrap();
