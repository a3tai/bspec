//! Controls (CTL) document implementation
//!
//! Internal controls and process safeguards
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.180060
...
impl CtlDocument {
    /// Create a new Controls document
...
impl BSpecDocument for CtlDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be CTL-something)
        if !self.metadata.id.starts_with("CTL-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'CTL-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for CtlDocument {
    fn default() -> Self {
        Self::new(
            "CTL-default".to_string(),
            "New Controls".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = CtlDocument::new(
            "CTL-test-001".to_string(),
            "Test Controls".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "CTL-test-001");
        assert_eq!(doc.metadata.title, "Test Controls");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Ctl);
    }

    #[test]
    fn test_validation() {
        let mut doc = CtlDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "CTL-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("CTL-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("CTL-ser-001"));

        // Test deserialization
        let deserialized: CtlDocument = serde_json::from_str(&json).unwrap();
