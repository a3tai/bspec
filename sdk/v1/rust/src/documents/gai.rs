//! Gains (GAI) document implementation
//!
//! Benefits and value customers receive
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.167413
...
impl BSpecDocument for GaiDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be GAI-something)
        if !self.metadata.id.starts_with("GAI-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'GAI-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for GaiDocument {
    fn default() -> Self {
        Self::new(
            "GAI-default".to_string(),
            "New Gains".to_string(),
            "Unknown".to_string(),
        )
    }
...
mod tests {
    ...
    fn test_new_document() {
        let doc = GaiDocument::new(
            "GAI-test-001".to_string(),
            "Test Gains".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "GAI-test-001");
        assert_eq!(doc.metadata.title, "Test Gains");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Gai);
    }

    #[test]
    fn test_validation() {
        let mut doc = GaiDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "GAI-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("GAI-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("GAI-ser-001"));

        // Test deserialization
        let deserialized: GaiDocument = serde_json::from_str(&json).unwrap();
