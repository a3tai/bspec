//! Revenue Streams (REV) document implementation
//!
//! How money is generated from customers
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.170698
...
impl RevDocument {
    /// Create a new Revenue Streams document
...
impl BSpecDocument for RevDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be REV-something)
        if !self.metadata.id.starts_with("REV-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'REV-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for RevDocument {
    fn default() -> Self {
        Self::new(
            "REV-default".to_string(),
            "New Revenue Streams".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = RevDocument::new(
            "REV-test-001".to_string(),
            "Test Revenue Streams".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "REV-test-001");
        assert_eq!(doc.metadata.title, "Test Revenue Streams");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Rev);
    }

    #[test]
    fn test_validation() {
        let mut doc = RevDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "REV-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("REV-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("REV-ser-001"));

        // Test deserialization
        let deserialized: RevDocument = serde_json::from_str(&json).unwrap();
