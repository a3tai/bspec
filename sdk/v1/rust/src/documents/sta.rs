//! Stakeholders (STA) document implementation
//!
//! Stakeholder mapping and management
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.180573
...
impl StaDocument {
    /// Create a new Stakeholders document
...
impl BSpecDocument for StaDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be STA-something)
        if !self.metadata.id.starts_with("STA-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'STA-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for StaDocument {
    fn default() -> Self {
        Self::new(
            "STA-default".to_string(),
            "New Stakeholders".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = StaDocument::new(
            "STA-test-001".to_string(),
            "Test Stakeholders".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "STA-test-001");
        assert_eq!(doc.metadata.title, "Test Stakeholders");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Sta);
    }

    #[test]
    fn test_validation() {
        let mut doc = StaDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "STA-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("STA-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("STA-ser-001"));

        // Test deserialization
        let deserialized: StaDocument = serde_json::from_str(&json).unwrap();
