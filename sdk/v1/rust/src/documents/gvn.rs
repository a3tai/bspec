//! Governance (GVN) document implementation
//!
//! Decision-making structure and oversight
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.179660
...
impl BSpecDocument for GvnDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be GVN-something)
        if !self.metadata.id.starts_with("GVN-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'GVN-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for GvnDocument {
    fn default() -> Self {
        Self::new(
            "GVN-default".to_string(),
            "New Governance".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = GvnDocument::new(
            "GVN-test-001".to_string(),
            "Test Governance".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "GVN-test-001");
        assert_eq!(doc.metadata.title, "Test Governance");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Gvn);
    }

    #[test]
    fn test_validation() {
        let mut doc = GvnDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "GVN-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("GVN-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("GVN-ser-001"));

        // Test deserialization
        let deserialized: GvnDocument = serde_json::from_str(&json).unwrap();
