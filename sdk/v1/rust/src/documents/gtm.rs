//! Go-to-Market (GTM) document implementation
//!
//! Launch and customer acquisition strategy
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.180790
...
impl GtmDocument {
    /// Create a new Go-to-Market document
...
impl BSpecDocument for GtmDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be GTM-something)
        if !self.metadata.id.starts_with("GTM-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'GTM-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for GtmDocument {
    fn default() -> Self {
        Self::new(
            "GTM-default".to_string(),
            "New Go-to-Market".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = GtmDocument::new(
            "GTM-test-001".to_string(),
            "Test Go-to-Market".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "GTM-test-001");
        assert_eq!(doc.metadata.title, "Test Go-to-Market");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Gtm);
    }

    #[test]
    fn test_validation() {
        let mut doc = GtmDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "GTM-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("GTM-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("GTM-ser-001"));

        // Test deserialization
        let deserialized: GtmDocument = serde_json::from_str(&json).unwrap();
