//! Jobs-to-be-Done (JTB) document implementation
//!
//! Specific outcomes customers hire products to achieve
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.166315
...
impl JtbDocument {
    /// Create a new Jobs-to-be-Done document
...
impl BSpecDocument for JtbDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be JTB-something)
        if !self.metadata.id.starts_with("JTB-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'JTB-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for JtbDocument {
    fn default() -> Self {
        Self::new(
            "JTB-default".to_string(),
            "New Jobs-to-be-Done".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = JtbDocument::new(
            "JTB-test-001".to_string(),
            "Test Jobs-to-be-Done".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "JTB-test-001");
        assert_eq!(doc.metadata.title, "Test Jobs-to-be-Done");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Jtb);
    }

    #[test]
    fn test_validation() {
        let mut doc = JtbDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "JTB-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("JTB-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("JTB-ser-001"));

        // Test deserialization
        let deserialized: JtbDocument = serde_json::from_str(&json).unwrap();
