//! Ecosystem (ECO) document implementation
//!
//! Partners, suppliers, distributors, and ecosystem players
//!
//! **GENERATED CODE - DO NOT EDIT MANUALLY**
//! Generated at: 2025-09-28T16:00:48.165175
...
impl EcoDocument {
    /// Create a new Ecosystem document
...
impl BSpecDocument for EcoDocument {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate ID format (should be ECO-something)
        if !self.metadata.id.starts_with("ECO-") {
            return Err(BSpecError::ValidationError(
                format!("ID should start with 'ECO-', got '{}'", self.metadata.id)
            ));
        }
...
impl Default for EcoDocument {
    fn default() -> Self {
        Self::new(
            "ECO-default".to_string(),
            "New Ecosystem".to_string(),
            "Unknown".to_string(),
        )
    }
}
...
mod tests {
    ...
    fn test_new_document() {
        let doc = EcoDocument::new(
            "ECO-test-001".to_string(),
            "Test Ecosystem".to_string(),
            "Test Owner".to_string(),
        );

        assert_eq!(doc.metadata.id, "ECO-test-001");
        assert_eq!(doc.metadata.title, "Test Ecosystem");
        assert_eq!(doc.metadata.owner, "Test Owner");
        assert_eq!(doc.document_type(), DocumentType::Eco);
    }

    #[test]
    fn test_validation() {
        let mut doc = EcoDocument::default();
        ...
        // Set required fields
        doc.metadata.id = "ECO-valid-001".to_string();
    ...
    fn test_serialization() {
        ...
        assert!(json.contains("ECO-ser-001"));

        // Test YAML serialization
        let yaml = doc.to_yaml().unwrap();
        assert!(yaml.contains("ECO-ser-001"));

        // Test deserialization
        let deserialized: EcoDocument = serde_json::from_str(&json).unwrap();
